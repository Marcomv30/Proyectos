-- RPC de seed demo para entorno local/desarrollo
-- Ejecutar despues de 082/083/084

begin;

create or replace function public.seed_caja_turnos_movimientos_demo()
returns text
language plpgsql
security definer
...
begin
  ...
  if v_user is null then
    raise exception 'No hay usuario para seed.';
  end if;
  ...
  values
    (v_turno_id, v_empresa_id, 'abrir', 'Turno demo abierto', jsonb_build_object('saldo_inicial', v_saldo_inicial), v_user),
    (v_turno_id, v_empresa_id, 'cerrar', 'Turno demo cerrado', jsonb_build_object('total_recaudado', v_total_recaudado, 'total_efectivo', v_total_efectivo), v_user);

  return format('Seed demo generado. empresa_id=%s turno_id=%s', v_empresa_id, v_turno_id);
end;
$$;

grant execute on function public.seed_caja_turnos_movimientos_demo() to authenticated, service_role;
