-- ============================================================
-- Fix: end_trans_id = 0 debe tratarse como turno abierto
-- Fusion usa 0 (no -1 ni NULL) para turno activo
-- ============================================================
...
BEGIN
  ...
  -- Buscar turno activo (S) que cubra este sale_id
  -- Fusion usa end_trans_id = 0 para turno abierto (además de NULL y -1)
  ...
  WHERE empresa_id    = p_empresa_id
    ...
    AND (
      ...
      OR end_trans_id <= 0      -- 0 y -1 = turno abierto
...
-- Retroactivo: asignar turno_id a ventas existentes sin turno
...
SET turno_id = (
  ...
  WHERE tc.empresa_id   = vc.empresa_id
    ...
    AND (tc.end_trans_id IS NULL OR tc.end_trans_id <= 0 OR tc.end_trans_id >= vc.sale_id)
  ORDER BY tc.start_trans_id DESC NULLS LAST
  LIMIT 1
)
WHERE turno_id IS NULL;

-- Inicializar control sync pisteros en un valor cercano al máximo actual
-- para que el primer sync traiga registros recientes (no desde cero)
UPDATE fusion_sync_control
SET ultimo_id = 835700   -- ~500 antes del max actual (836229)
WHERE empresa_id = 1
  AND tabla_fusion = 'ssf_tkt_trx_header'
  AND ultimo_id = 0;
