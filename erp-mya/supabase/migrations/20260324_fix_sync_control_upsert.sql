-- ============================================================
-- MYA ERP — Fix: fusion_sync_control usa UPSERT en lugar de UPDATE
-- El UPDATE fallaba silenciosamente cuando no existía la fila
-- (ej: empresa nueva sin fila inicial en fusion_sync_control).
-- ============================================================
...
BEGIN
  ...
  -- Turno activo al que pertenece la venta
  ...
  -- Asiento contable automático (si hay cuenta y período configurados)
  ...
  -- Actualizar control de sync (UPSERT — crea la fila si no existe)
  INSERT INTO fusion_sync_control (empresa_id, tabla_fusion, ultimo_id, ultima_sync, registros_hoy, errores_consec)
  VALUES (p_empresa_id, 'ssf_pump_sales', p_sale_id, NOW(), 1, 0)
  ON CONFLICT (empresa_id, tabla_fusion) DO UPDATE
    SET ultimo_id      = GREATEST(fusion_sync_control.ultimo_id, EXCLUDED.ultimo_id),
        ultima_sync    = EXCLUDED.ultima_sync,
        registros_hoy  = fusion_sync_control.registros_hoy + 1,
        errores_consec = 0;
