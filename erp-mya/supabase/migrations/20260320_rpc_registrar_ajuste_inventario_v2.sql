-- Ajuste inventario v2
-- Permite registrar movimiento con fecha indicada y opcionalmente omitir
-- la creación inmediata de asiento para luego consolidar un asiento neto.
...
BEGIN
  ...
  IF NOT COALESCE(p_generar_asiento, TRUE) THEN
  ...
  SELECT cuenta_inventario_id INTO v_cuenta_inv_id
  FROM inv_productos
  WHERE id = p_producto_id;
  ...
  IF v_monto > 0 AND v_cuenta_inv_id IS NOT NULL AND v_cuenta_ajuste IS NOT NULL THEN
    BEGIN
      ...
      INSERT INTO asientos (
        ...
        'Ajuste inventario - ' || v_producto.descripcion || COALESCE(' - ' || p_referencia, ''),
        v_numero_fmt, p_categoria_id, 'CONFIRMADO'
      )
      RETURNING id INTO v_asiento_id;

      SELECT cuenta_base_id INTO v_base_inv_id
      FROM plan_cuentas_empresa
      WHERE id = v_cuenta_inv_id;

      SELECT cuenta_base_id INTO v_base_ajuste_id
      FROM plan_cuentas_empresa
      WHERE id = v_cuenta_ajuste;
