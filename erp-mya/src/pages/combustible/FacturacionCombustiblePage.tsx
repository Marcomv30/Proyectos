...
const STYLES = `
  ...
  .comb-fact-modal-backdrop { position:fixed; inset:0; background:rgba(2,6,23,.66); display:flex; align-items:center; justify-content:center; padding:18px; z-index:60; }
...
export default function FacturacionCombustiblePage({ empresaId }: Props) {
  ...
  useEffect(() => {
    ...
    const nextCondicion = selectedCliente ? '02' : '01'
    const nextPlazo = selectedCliente ? Number(selectedCliente.dias_credito || 0) : 0
    if (
  ...
  }, [draft, tipoDocumentoAuto, selectedCliente])
  ...
  const tituloPreview = marcadas.size > 1
    ...
    }, [draft?.condicionVenta, totalAFacturar, config.medio_pago_defecto])
