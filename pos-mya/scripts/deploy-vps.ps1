...
Write-Host 'Subiendo archivos raíz...'
& $PSCP -batch -P $VPS_PORT -pw $VPS_PASS ($BUILD + '\index.html') ($TARGET + ':' + $REMOTE + '/')
& $PSCP -batch -P $VPS_PORT -pw $VPS_PASS ($BUILD + '\favicon.svg') ($TARGET + ':' + $REMOTE + '/') 2>$null
& $PSCP -batch -P $VPS_PORT -pw $VPS_PASS ($BUILD + '\icons.svg') ($TARGET + ':' + $REMOTE + '/') 2>$null
Get-ChildItem ($BUILD + '\*.png') | ForEach-Object {
...
Write-Host 'URL en producción: https://pos-mya.visionzn.net'
